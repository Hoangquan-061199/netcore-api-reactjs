...
const FormAccount = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const rs: any = await updateAccLoginApi(req).unwrap()
  ...
  useEffect(() => {
    const url = getImageDomain(data?.urlPicture, null)
    setImageUrl(url)
  }, [data?.urlPicture])
          ...
          <Col className='gutter-row' span={4}>
            <div style={{ height: 'auto', width: '80%', aspectRatio: 1 }}>
              <Form.Item name='urlPicture'>
                ...
                >
                  {imageUrl ? (
                    <img src={imageUrl} alt='avatar' style={{ width: '100%', aspectRatio: 1, borderRadius: '50%' }} />
                  ) : (
                    uploadButton
                  )}
                </Upload>
